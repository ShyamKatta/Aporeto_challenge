...
while(1):
	r = requests.get(url,auth = ('shyamkatta','db873d692c431fc9e28a7c19c381d7d01663c8cb'),headers={"Content-Type": "application/json"})
	jsonObj  = r.json()
...
iter=1
url = "https://api.github.com/users/"+uname+"/repos?per_page=100"
...
while(1):
	r = requests.get(url+"&page="+str(iter),auth = ('shyamkatta','db873d692c431fc9e28a7c19c381d7d01663c8cb'),headers={"Content-Type": "application/json"})
	jsonObj  = r.json()
	if (r.status_code==200  or r.status_code == 304) and (len(jsonObj)!=0):
...
for repo in uniqueRepoSet:
	...
	if r.status_code==200 or r.status_code == 304:
		...
		if jsonObj['fork']:
			...
			# add source to repo
			repo_id = jsonObj['source']['full_name']
			mytuple = ("fork_source_repo", repo_id)
			uniqueRepoEventSet.add(mytuple)
...
for i in temp_new_list:
	if (1):#i[0]=="org_repo" or i[0]=="user_repo"):
		...
		while(1):
			...
			r = requests.get(url+"&page="+str(iter)+"&per_page=100",auth = ('shyamkatta','db873d692c431fc9e28a7c19c381d7d01663c8cb'),headers={"Content-Type": "application/json"})
			jsonObj  = r.json()
			if (r.status_code==200 or r.status_code == 304) and (len(jsonObj)!=0):
				for res in jsonObj:
